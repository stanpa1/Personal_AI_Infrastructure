...
Extended with Notion project management instructions.
"""
...
def get_notion_projects_context() -> str:
    """
    Get current projects from Notion for context.
    """
    try:
        from notion_project_handler import get_all_projects
        projects = get_all_projects()

        if not projects:
            return ""

        lines = ["\n## AKTYWNE PROJEKTY W NOTION"]
        for p in projects:
            line = f"- {p['name']}"
            if p.get('priority'):
                line += f" [{p['priority']}]"
        ...
        logger.warning(f"Could not load Notion projects: {e}")
        return ""
...
def load_pai_context() -> str:
    ...
    # Base system prompt with Notion integration
...
Możesz pomagać użytkownikowi zarządzać projektami w Notion. System automatycznie przetworzy Twoje odpowiedzi.

Gdy użytkownik wspomni o aktualizacji projektu, rozpoznaj intencję i odpowiedz naturalnie.
System automatycznie zaktualizuje Notion na podstawie wiadomości użytkownika.

Przykłady:
1. "antena gsm - zrobione" → System oznaczy projekt jako Done
2. "update matryce: spotkanie z Nikiem" → System zaktualizuje Next Action
3. "dodaj projekt: zakup laptopa, dom, wysoki" → System utworzy nowy projekt

Twoja rola:
- Potwierdź, że rozumiesz co użytkownik chce zrobić
- Jeśli intencja jest niejasna, zapytaj o szczegóły
- Przy "zrobione" zapytaj opcjonalnie o następny krok
- Przy nowym projekcie potwierdź szczegóły

NIE musisz generować specjalnych tagów - system sam wykryje intencję z wiadomości użytkownika.
""")
    ...
    notion_context = get_notion_projects_context()
    if notion_context:
        context_parts.append(notion_context)

    # Load active projects from files (backup)
