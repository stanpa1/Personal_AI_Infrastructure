...
Extended with Notion project management.
"""
...
from notion_project_handler import process_project_update, detect_project_intent
...
def handle_project_update(user_text: str, chat_id: int, result: dict) -> None:
    """
    Process potential project update from user message.
    Detects intent and updates Notion if applicable.
    """
    try:
        # Check for project-related intent
        intent = detect_project_intent(user_text)

        if not intent.get('type'):
            logger.debug("No project intent detected")
            return

        logger.info(f"Project intent detected: {intent}")

        # Process the update
        notion_result = process_project_update(user_text)

        if notion_result.get('updated'):
            # Send confirmation to user
            confirmation = notion_result.get('confirmation', 'Zaktualizowano projekt.')
            logger.info(f"Notion update successful: {confirmation}")

            # Send confirmation message
            send_result = send_message(chat_id, f"✅ {confirmation}")
            result['notion_update'] = {
                'success': True,
                'action': notion_result.get('action'),
                'project': notion_result.get('project'),
                'confirmation_sent': send_result.get('success', False)
            }

        elif notion_result.get('error'):
            logger.warning(f"Notion update failed: {notion_result['error']}")
            result['notion_update'] = {
                'success': False,
        ...
        logger.exception(f"Error in project update handler: {e}")
        result['notion_update'] = {
            'success': False,
            'error': str(e)
        }
        ...
        if should_process_with_claude(text_to_check) and chat_id:
            ...
            if should_respond and claude_response:
                ...
                if send_result.get('success'):
                    ...
                    # === NOTION PROJECT HANDLER ===
                    # After sending AI response, check for project updates
                    handle_project_update(text_to_check, chat_id, result)

                else:
                    logger.error(f"Failed to send Telegram response: {send_result.get('error')}")

        # === DIRECT PROJECT COMMANDS (without @pai) ===
        # Also check for direct project commands like "matryce - zrobione"
        elif chat_id and text_to_check:
            intent = detect_project_intent(text_to_check)
            if intent.get('type'):
                logger.info(f"Direct project command detected: {intent}")
                handle_project_update(text_to_check, chat_id, result)
