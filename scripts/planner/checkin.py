#!/usr/bin/env python3
"""
PAI Planner - Daily Check-in System
Runs at 6:30 (morning) and 19:00 (evening)
Uses Notion API + DeepSeek for check-in messages.
"""
...
def get_notion_projects() -> List[Dict]:
...
def update_last_review(project_id: str):
    """Update Last Review date for a project (for future use)"""
    # TODO: Implement if needed
    pass


def generate_checkin_with_deepseek(projects: List[Dict], checkin_type: str) -> Optional[str]:
    ...
    for p in projects:
        ...
        if p['deadline'] and p['deadline'] <= tomorrow:
...
def generate_fallback_message(projects: List[Dict], checkin_type: str) -> str:
...
def run_checkin(checkin_type: Optional[str] = None):
    ...
    if checkin_type is None:
        ...
        checkin_type = "morning" if hour < 12 else "evening"
    ...
    if not projects:
        message = f"{'🌅' if checkin_type == 'morning' else '🌙'} Check-in - nie udało się pobrać projektów z Notion."
        send_checkin_message(message)
        return

    # Generate message with DeepSeek
    message = generate_checkin_with_deepseek(projects, checkin_type)

    # Fallback if DeepSeek fails
    if not message:
        logger.warning("DeepSeek failed, using fallback message")
        message = generate_fallback_message(projects, checkin_type)
